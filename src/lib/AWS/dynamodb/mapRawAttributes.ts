...
export const mapRawAttribute = (attribute: DynamoDB.AttributeValue): any => {
    const typed = mapValues(attribute, (value, key: keyof DynamoDB.AttributeValue): any => {
        switch (key) {
            case 'M':
    ...
    return Object.values(typed).shift()
}
